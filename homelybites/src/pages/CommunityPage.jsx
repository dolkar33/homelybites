...
const CommunityPage = () => {
  ...
  const fetchSuggestions = async (page = 1, append = false) => {
    ...
    try {
      const res = await axiosInstance.get(
        `/api/suggestions/match/?page=${page}`,
      );
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const paramsArr = [];
        if (activeCategory)
          paramsArr.push(`category=${encodeURIComponent(activeCategory)}`);
        ...
        try {
          ...
          try {
            let communityUser;
            if (userId) {
              const cuRes = await axiosInstance.get(
                `/api/community-users/by-id/${encodeURIComponent(userId)}/`,
              );
              communityUser = cuRes?.data;
            }
            const postsCount =
              typeof communityUser?.posts === "number"
                ? communityUser.posts
                : Array.isArray(communityUser?.posts)
                  ? communityUser.posts.length
                  : 0;
            setCurrentUser({
              ...baseUser,
  ...
  const handleLike = async (postId) => {
    ...
    try {
      ...
      const serverLikes =
        res?.data?.likes ?? res?.data?.like_count ?? res?.data?.likes_count;
          ...
          prev.map((post) =>
            post.id === postId
              ? {
                  ...post,
                  isLiked:
                    serverLiked !== undefined ? serverLiked : post.isLiked,
                  likes:
                    typeof serverLikes === "number" ? serverLikes : post.likes,
                }
              : post,
          ),
        );
  ...
  const handleSave = async (postId) => {
    try {
      ...
      const nextSaved =
        typeof res?.data?.saved === "boolean" ? res.data.saved : undefined;
      setPosts((posts) =>
        posts.map((post) =>
          post.id === postId
            ? {
  ...
  return (
    <div className="min-h-screen flex flex-col bg-gray-50">
      ...
      <div className="flex-1 px-[100px] py-4 sm:py-6">
        <div className="max-w-7xl mx-auto">
          <div className="grid grid-cols-12 gap-4 sm:gap-6">
                      ...
                      <div className="bg-white rounded-2xl sm:rounded-3xl shadow-lg border border-gray-100 p-4 sm:p-6 mt-0 mb-0">
                        ...
                        <nav className="space-y-3 sm:space-y-4">
                          <button
                            ...
                            onClick={() =>
                              navigate(
                                `/community?view=feed${activeCategory ? `&category=${encodeURIComponent(activeCategory)}` : ""}`,
                              )
                            }
                          ...
                          <button
                            ...
                            onClick={() =>
                              navigate(
                                `/community?view=mypost${activeCategory ? `&category=${encodeURIComponent(activeCategory)}` : ""}`,
                              )
                            }
                            className={`flex items-center gap-2 sm:gap-3 transition-colors p-2 rounded-lg sm:rounded-xl w-full text-left ${
                              currentView === "mypost"
                          ...
                          <button
                            ...
                            onClick={() =>
                              navigate(
                                `/community?view=saved${activeCategory ? `&category=${encodeURIComponent(activeCategory)}` : ""}`,
                              )
                            }
                            className={`flex items-center gap-2 sm:gap-3 transition-colors p-2 rounded-lg sm:rounded-xl w-full text-left ${
                              currentView === "saved"
              ...
              <div className="sticky top-0 space-y-4 sm:space-y-6">
                ...
                <div className="bg-white rounded-2xl sm:rounded-3xl shadow-lg border border-gray-100 p-4 sm:p-6">
                  <div className="text-center mb-0 sm:mb-0">
                    ...
                    <div className="text-center mt-2 text-base sm:text-lg font-normal text-gray-800">
            ...
            <div className="col-span-12 md:col-span-6">
              <div
                className={`space-y-4 sm:space-y-6 transition-opacity duration-200 ${dimFade || isFetching ? "opacity-95" : "opacity-100"}`}
              >
            ...
            <div className="col-span-3 hidden md:block">
              <div
                className="sticky top-0 h-screen overflow-y-auto pr-4 -mr-4"
                style={{
              ...
              >
                ...
                <div className="bg-white rounded-2xl sm:rounded-3xl border border-gray-100 p-4 sm:p-6 mt-4">
                  <div className="flex items-center justify-between mb-4 sm:mb-6">
                    ...
                    >
                      <RefreshCw
                        className={`w-4 h-4 ${sugLoading ? "animate-spin" : ""}`}
                      />
      ...
      {showDeleteModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          <div
            ...
            onClick={deletingLoading ? undefined : cancelDelete}
          />
            ...
            <div className="flex items-start gap-3">
              <div className="w-10 h-10 rounded-full bg-red-50 text-red-500 flex items-center justify-center flex-shrink-0">
                ...
                >
                  <path
                    fillRule="evenodd"
                    d="M10 18a8 8 0 100-16 8 8 0 000 16zm-1-5a1 1 0 102 0 1 1 0 00-2 0zm1-7a1 1 0 00-1 1v5a1 1 0 102 0V7a1 1 0 00-1-1z"
                    clipRule="evenodd"
                  />
                </svg>
              </div>
